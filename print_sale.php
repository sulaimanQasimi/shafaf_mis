<?php
    ...
    // Get sale_id from URL parameter
    if(!isset($_GET['sale_id']) || empty($_GET['sale_id'])) {
        header("location:selled_page.php");
        exit;
    }
    ...
    // Get sale major information
    $sql_query_sale = mysqli_query($connection,"SELECT 
        sale_major.id AS bill_number,
        COALESCE(customers.full_name, '') AS customer_name,
        COALESCE(customers.phone_number, '') AS customer_phone,
        COALESCE(customers.address, '') AS customer_address,
    ...
    WHERE sale_major.id = '$sale_id'");
    
    if(mysqli_num_rows($sql_query_sale) == 0) {
        header("location:selled_page.php");
        exit;
    }
    
    $sale_data = mysqli_fetch_assoc($sql_query_sale);
    
    // Get sale minor items
    $sql_query_items = mysqli_query($connection,"SELECT * FROM `view_list_of_sales` where sale_major_id='$sale_id' ORDER BY id");
    
    // Calculate totals
    $total_sold_price = $sale_data["total_sold_price"];
    $total_reciepts_price = $sale_data["total_reciepts_price"];
    $total_remain = $total_sold_price - $total_reciepts_price;
?>
<!DOCTYPE html>
<html lang="en">
<head>
            ...
            <div class="page-title-box print-display">
                <div class="page-title-right print-display">
                    <ol class="breadcrumb m-0">
                        ...
                        <li class="breadcrumb-item active">چاپ فاکتور فروش</li>
                    </ol>
                </div>
                <p class="page-title">چاپ فاکتور فروش</p>
            </div>
            <!-- end page title -->
                            ...
                            <div class="row">
                                ...
                                <div class="col-md-6 text-right">
                                    ...
                                    <p class="invoice-date-print" style="margin-top: 10px; font-size: 14px; color: #7f8c8d;">
                                        <span>تاریخ: <?php echo $sale_data["sale_date"]; ?></span>
                                    </p>
                                    <p class="invoice-date-print" style="margin-top: 5px; font-size: 14px; color: #7f8c8d;">
                                        <span>شماره بل: <?php echo $sale_data["bill_number"]; ?></span>
                                    </p>
                                </div>
                            </div>
                        </div>
                                    ...
                                    <table class="table mt-4 table-sm table-centered">
                                        ...
                                        <tbody>
                                            <?php
                                                ...
                                                {
                                                    $profit = $item["sale_rate"] - $item["purchase_price_t"];
                                                    $row_total = $item["sale_rate"] * $item["amount"];
                                            ?>
                                            <tr>
                                                ...
                                                <td><?php echo number_format($item["purchase_price_t"], 2); ?></td>
                                                <td><?php echo number_format($item["sale_rate"], 2); ?></td>
                                        ...
                                        <tfoot>
                                            <tr>
                                                <th colspan="9">مجموع فروش</th>
                                                <th><?php echo number_format($total_sold_price, 2); ?></th>
                                            </tr>
                                            <tr>
                                                <th colspan="9">مجموع رسید</th>
                                                <th><?php echo number_format($total_reciepts_price, 2); ?></th>
                                            </tr>
                                            <tr>
                                                <th colspan="9">مجموع باقی</th>
